...
const Castfun = (castee) => {
  let url = "personDetail5445.html?id=" + encodeURIComponent(castee.id);
  return `<div class="Now_playing_movies castdiv" >
    <a class="posterlink" href="${url}"> <img class="poster" data-id="${castee.id}" src="https://image.tmdb.org/t/p/w500/${castee.profile_path}" loading="lazy" 
        onerror="this.onerror=null;this.src='https://moviedb.troyenft.xyz/resources/D moviesand tv show.png';"
...
const Trailerfunc = function (id) {
  return `<iframe class="youtubePlayer" src="https://autoembed.to/tv/tmdb/${id}-1-1?trailer=1" width="100%" height="100%" loading="lazy"  frameborder="0" allowfullscreen></iframe>`;
};
...
const recommTvshowFun = (mov) => {
  let url = "TvshowsDetails5445-2.html?id=" + encodeURIComponent(mov.id);
...
const simimarTvshowfun = (movie) => {
  let url = "TvshowsDetails5445-2.html?id=" + encodeURIComponent(movie.id);
